...
| 2026-06-12 | **Phases 0–3 built.** Schema groundwork (link annotations, `timeframeNormalized`, `sourceQuote`/`sourceProvider`, bodyParts CRITICAL guard); anatomy registries (`anatomy-regions.ts` 50 regions / 465 meshes, `anatomy-aliases.ts`, `careplan/bodyparts.ts`); full `src/lib/careplan/` logic module (`types`, `timeframe` + 31 fixtures, `certainty`, `dedup`, `context`, `assembly`, `merge` + fixtures, `store`, `plain-language`, `journey`, `highlights`, `import-hook`) — 123 unit tests; import hooks (context blob transport in process-request body, `recommendations-processing` LangGraph node + `consumesCarePlanContext`, finalizer merge hook returning deltas, `originKind` flag); chat + first mutating MCP tool (`createCarePlanTask`, strict-ownership gate, write audit); UI shell (`/med/p/[profile]/care-plan` route, item card, potential/empty state, dashboard section, nav item, ported `CareJourneyTimeline` from aouros, i18n `careplan.*`); 3D wiring (`Body.svelte` `carePlanRegions` + `carePlanRegionClick`) and post-import `CarePlanUpdate` summary. **Open conflicts resolved:** C1 (recommendations node), C2 (context blob in request body, not AES-wrapped), C3 (merge validates link ids defensively), C4 (`ChatContext.carePlanContext` + `AskAboutEvent` reuse, no `ChatContextItem` union), C5 (`anatomy-integration.ts` unified with region registry), C6 (lazy `DocumentType.careplan` singleton). i18n key convention pinned `medical.icd10.<code>.plain`. |
| 2026-06-12 | **Phases 4–5 built (Trust layer + Chat loop).** Row 15: shared `ProvenanceReveal.svelte` + pure `careplan/provenance.ts` (doc/chat/user paths, no-quote fallback, conflict line) — 9 unit tests; wired into `CarePlanItemCard` at item + per-task level with a `docTitles` map from the page. Row 16: `SnoozeDialog.svelte` over `Modal` capturing reason+note+duration → existing `applyUserTaskAction`. Row 17: distinct per-type event markers (●/`mic`/✓/◎/○) on `CareJourneyTimeline` from `buildJourneyEvents`. Row 18: `showCertaintyLabelsInline` **user-level** setting in `profiles.settings` jsonb (migration 20260414) via new `/v1/user/settings` endpoint + `settings` on the `User` store + settings-page toggle; card renders the inline label when on. Row 19: AI emits a structured `suggestedAction` (mirrors the `widgets` field through `chat.json` schema → server metadata → `chat-manager` → `AIChatSidebar` footer); tapping it calls `addUserTask(..., {sourceMessageId})` directly (the tap is consent — no autonomous tool call, `createCarePlanTask` MCP tool unchanged); created tasks render the chat provenance path and survive re-import. Also fixed a latent M0 regression: the joint NL synonyms in `anatomy-integration.ts` now include unsided ligament/cartilage meshes. **Still deferred:** §2.1–2.6 expansion (Care Team rail, goal sparklines, multi-profile UX, Care Plan share, local notifications), goal-card rendering + impact tags, ProfileDashboard 3D hero (model lives on the Care Plan page), the standalone conflict-review flow (reveal shows the disagreement line only), ProvenanceReveal side-panel-on-mobile, region-rollup anatomical review pass. |
