...
export async function exportUserData(
	...
	const profiles = allProfiles.filter(
		(p: any) => p.profiles?.auth_id === userInfo.userId || p.profiles?.owner_id === userInfo.userId,
	);
	...
	for (const profile of profiles) {
		...
		const docsRes = await apiFetch(`/v1/med/profiles/${profileId}/documents?full=true`);
		const docsData = await docsRes.json();
		const encrypted: DocumentEncrypted[] = Array.isArray(docsData) ? docsData : [];
		...
		for (const doc of decrypted) {
			...
			for (const att of attachments) {
				...
				onProgress?.(`Downloading attachment: ${attStoragePath.split('/').pop()}...`);
				try {
					const attRes = await apiFetch(
						`/v1/med/profiles/${profileId}/attachments?path=${encodeURIComponent(attStoragePath)}`,
					);
					if (!attRes.ok) {
						console.warn('Failed to download attachment, status:', attRes.status, attStoragePath);
						continue;
					}
					const encryptedText = await attRes.text();

					// Decrypt using document key
					const [decryptedJson] = await decrypt([encryptedText], (doc as any).key);
