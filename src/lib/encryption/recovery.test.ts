...
describe("Recovery Key Generation", () => {
  it.skipIf(!hasCrypto)(
    "should generate a recovery key in the correct format",
    () => {
...
-----END PRIVATE KEY-----`;
  ...
  it.skipIf(!hasCrypto)("should encrypt and decrypt private key", async () => {
    ...
    const encrypted = await encryptWithRecoveryKey(
      testPrivateKeyPEM,
      recoveryKey,
    );
  ...
  it.skipIf(!hasCrypto)(
    "should reject invalid key format during encryption",
    async () => {
      await expect(
  ...
  it.skipIf(!hasCrypto)(
    "should reject invalid key format during decryption",
    async () => {
      await expect(
...
describe("Recovery Key Hashing", () => {
  it.skipIf(!hasCrypto)(
    "should create consistent hash for same key",
    async () => {
      const recoveryKey = generateRecoveryKey();
  ...
  it.skipIf(!hasCrypto)(
    "should create different hashes for different keys",
    async () => {
      const key1 = generateRecoveryKey();
      const key2 = generateRecoveryKey();
