 ...
 */
export async function generateRecoveryPDF(
  options: RecoveryDocumentOptions,
): Promise<Uint8Array> {
  const {
    email,
    recoveryKey,
 ...
 */
export async function downloadRecoveryPDF(
  options: RecoveryDocumentOptions,
): Promise<void> {
 ...
 */
export async function printRecoveryPDF(
  options: RecoveryDocumentOptions,
): Promise<void> {
  const pdfBytes = await generateRecoveryPDF(options);
