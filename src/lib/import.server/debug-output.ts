  ...
  try {
    ...
    const data = {
      ...
      metadata: {
        ...
        totalPages: extractionResults.reduce(
          (sum, r) => sum + (r.pages?.length || 0),
          0,
        ),
...
export function saveCompleteWorkflow(
  ...
  try {
    ...
    const data = {
      ...
      extractionResults,
      analysisResults,
      ...(config.includeWorkflowState && workflowState
        ? { workflowState }
        : {}),
      metadata: {
...
export function saveDocumentWorkflow(
  ...
  try {
    ...
    writeFileSync(filepath, json, "utf-8");
    console.log(
      `📁 [Debug] Saved document ${documentIndex} workflow to: ${filepath}`,
    );
  } catch (error) {
