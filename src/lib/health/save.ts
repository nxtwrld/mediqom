...
function getFieldItems(
  fieldKey: string,
): { key: string; type: string; unit?: string }[] {
  const def = definitions.find((d: any) => d.key === fieldKey);
  if (!def || def.type !== "time-series") return [];
  return def.items || [];
}
...
export async function saveHealthProfile(
  options: SaveHealthProfileOptions,
  ...
  try {
    ...
    for (const field of TIME_SERIES_FIELDS) {
      ...
      if (isCompoundTimeSeries(field)) {
        ...
        if (typeof value === "object" && !Array.isArray(value)) {
          ...
          const items = getFieldItems(field).filter(
    ...
    healthLogger.error("Failed to save health profile", {
      profileId,
      error: errorMessage,
    });
