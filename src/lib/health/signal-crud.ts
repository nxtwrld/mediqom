...
export async function addSignalEntry(
	...
	try {
		...
		healthLogger.info('Signal entry added', { profileId, signal, date: entry.date });
		return { success: true };
	} catch (error) {
		const errorMessage = error instanceof Error ? error.message : String(error);
		healthLogger.error('Failed to add signal entry', { profileId, signal, error: errorMessage });
...
export async function updateSignalEntry(
	...
	try {
		...
		signalData.values.sort(
			(a: Signal, b: Signal) => new Date(b.date).getTime() - new Date(a.date).getTime()
		);
...
export async function deleteSignalEntry(
	...
	try {
		...
		const entry = signalData.values[index];
		if (entry.source !== 'input' && entry.refId) {
			return { success: false, error: 'Cannot delete document-sourced entries' };
		}

		// Remove the entry - create new array to trigger reactivity
		const newValues = [...signalData.values.slice(0, index), ...signalData.values.slice(index + 1)];
 ...
 */
export async function getSignalValues(profileId: string, signal: string): Promise<Signal[]> {
	try {
		const document = await getHealthDocument(profileId);
		return document?.content?.signals?.[signal]?.values || [];
	} catch (error) {
		healthLogger.error('Failed to get signal values', { profileId, signal, error });
		return [];
	}
}
