...
export class StreamingTranscriptionClient {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      console.log(
        "[StreamingClient] Connecting to",
        this.tokenData.provider,
        "with tokenType:",
        this.tokenData.metadata.tokenType,
      );
  ...
  private async fetchToken(): Promise<TokenResponse> {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        error.message || error.error || "Failed to get streaming token",
      );
    }
  ...
  private scheduleTokenRefresh(): void {
    ...
    console.log(
      "[StreamingClient] Token refresh scheduled in",
