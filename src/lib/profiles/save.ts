...
export async function saveProfileDocument(
  ...
  try {
    ...
    profileLogger.error("Failed to save profile document", {
      profileId,
      error: errorMessage,
    });
...
export async function saveProfileChanges(
  ...
  const vcardChanged =
    JSON.stringify(editingSnapshot.vcard) !==
    JSON.stringify(originalSnapshot.vcard);
  const healthChanged =
    JSON.stringify(editingSnapshot.health) !==
    JSON.stringify(originalSnapshot.health);
  const insuranceChanged =
    JSON.stringify(editingSnapshot.insurance) !==
    JSON.stringify(originalSnapshot.insurance);
  ...
  try {
      ...
      if (profileResult.success && profileResult.fullName) {
        try {
          const response = await apiFetch(
            `/v1/med/profiles/${editingProfile.id}`,
            {
              method: "PATCH",
    ...
    profileLogger.info("Profile changes saved successfully", {
      profileId: editingProfile.id,
    });
    return { success: true };
  } catch (error) {
