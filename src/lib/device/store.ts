...
function createDeviceStore() {
	...
	function init() {
		...
		const updateOrientation = () => {
			...
			update((s) => ({ ...s, orientation, orientationAngle: angle, isLandscape }));
		};

		if (window.screen?.orientation) {
			window.screen.orientation.addEventListener('change', updateOrientation);
			cleanupFns.push(() => window.screen.orientation.removeEventListener('change', updateOrientation));
		}
		window.addEventListener('orientationchange', updateOrientation);
		cleanupFns.push(() => window.removeEventListener('orientationchange', updateOrientation));

		// ---- Online/Offline ----
		const updateOnline = () => update((s) => ({ ...s, isOnline: navigator.onLine }));
		...
		const navConnection = (navigator as unknown as { connection?: EventTarget }).connection;
		if (navConnection) {
			navConnection.addEventListener('change', updateConnection);
			cleanupFns.push(() => navConnection?.removeEventListener('change', updateConnection));
		}

		// ---- Page Visibility ----
		const updateVisibility = () => update((s) => ({ ...s, isVisible: !document.hidden }));
		document.addEventListener('visibilitychange', updateVisibility);
		cleanupFns.push(() => document.removeEventListener('visibilitychange', updateVisibility));
