...
export const scansAvailable = derived(
  subscription,
  ($sub) => $sub?.scans_available ?? 0,
);

export const hasScans = derived(scansAvailable, ($available) => $available > 0);

export const canCreateProfile = derived(
  subscription,
  ($sub) => $sub?.can_create_profile ?? false,
);

export const isFreeTier = derived(
  subscription,
  ($sub) => $sub?.tier_id === "free",
);
...
export async function loadSubscription(fetchFn?: typeof fetch): Promise<void> {
  ...
  try {
    ...
    const data = await apiGet<SubscriptionWithUsage>(
