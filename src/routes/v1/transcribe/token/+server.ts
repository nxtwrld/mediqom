...
async function generateDeepgramToken(
  ...
  if (!response.ok) {
    ...
    throw new Error(
      errorData.err_msg ||
        `Failed to create Deepgram access token (${response.status})`,
    );
  }
  ...
  if (!accessToken) {
    logger.audio.error("Deepgram API returned empty access token", {
      tokenData,
    });
...
export const POST: RequestHandler = async ({ request, locals }) => {
  ...
  try {
    ...
    logger.audio.debug("Token request received", {
      provider,
      language,
      options,
    });
    ...
    if (provider !== "deepgram" && provider !== "auto") {
      error(
        501,
        "Only Deepgram provider is supported for client-side streaming.",
      );
    }
