...
import { verifyRecoveryKeyHash, hashRecoveryKey } from '$lib/encryption/recovery';
...
export const POST: RequestHandler = async ({ request }) => {
    ...
    try {
        ...
        if (privateKeys.recovery_key_hash) {
            const isValid = await verifyRecoveryKeyHash(recoveryKey, privateKeys.recovery_key_hash);
            if (!isValid) {
