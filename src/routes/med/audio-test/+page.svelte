<script lang="ts">
	...
	let transportMode = $state<'batch' | 'live' | 'deepgram'>('batch');
	...
	async function connectLiveSSE(): Promise<boolean> {
		...
		return new Promise((resolve) => {
			...
			try {
				...
				addLog('error', 'Live SSE', liveSocketError);
				resolve(false);
				return;
			}
			...
			liveEventSource.addEventListener('open', () => {
				...
				addLog('success', 'Live SSE', 'SSE connected');
			});
			...
			liveEventSource.addEventListener('message', (event) => {
				try {
					...
					if (payload.type === 'connected') {
						...
						addLog('success', 'Live SSE', `Session established: ${liveSessionId}`);
						resolve(true);
						...
						addLog('info', 'Live SSE', `Received ${payload.type} transcript`, {
							seq: payload.seq,
						...
						addLog('error', 'Live SSE', liveSocketError || 'Unknown error');
					}
				} catch (err) {
					addLog('error', 'Live SSE', 'Failed to parse SSE message', {
			...
			const handleClose = (reason: string) => {
				liveSocketReady = false;
				liveSessionId = null;
				addLog('warning', 'Live SSE', reason);
			};
	...
	async function startRecording() {
		...
		if (connectionPromise) {
			connectionPromise.then((connected) => {
				if (!connected) {
					const mode = transportMode === 'deepgram' ? 'Deepgram' : 'Live SSE';
	...
	async function transcribeChunk(audioData: Float32Array) {
		...
		if (transportMode === 'live') {
			try {
				...
				if (!ok || !liveSessionId) {
					addLog('warning', 'Live SSE', 'SSE not ready, falling back to batch');
					await transcribeChunkBatch(audioData);
					return;
				}
				...
				addLog('info', 'Live SSE', 'Sent audio frame to live endpoint', {
					samples: audioData.length,
					seq
				});
				return;
			} catch (err) {
				addLog('error', 'Live SSE', 'Failed to send audio over SSE, falling back', {
					error: (err as Error).message
				});
				await transcribeChunkBatch(audioData);
				return;
	...
	<header class="page-header">
		...
		<div class="status-bar">
			...
			<div class="status-item">
				...
				<span class="value" style="color: {isRecording ? 'var(--color-positive)' : 'var(--color-text-secondary)'}">
					{isRecording ? '● REC' : '○ Idle'}
				</span>
			</div>
			<div class="status-item">
				<span class="label">Transcribing:</span>
				<span class="value" style="color: {isTranscribing ? 'var(--color-info)' : 'var(--color-text-secondary)'}">
		...
		<div class="settings-grid">
			...
			<div class="setting-group">
				...
				<select
					...
					disabled={isRecording && (transportMode === 'live' || transportMode === 'deepgram')}
				>
...
<style>
	...
	.setting-group textarea {
		padding: var(--ui-pad-small) 0.75rem;
		border: 1px solid var(--color-border);
	...
	.setting-group textarea:focus {
		outline: 2px solid var(--color-interactivity);
		outline-offset: 0;
	}
	...
	.panel-header {
		...
		align-items: center;
		padding: 0.75rem var(--ui-pad-medium);
	...
	.transcript-segment {
		...
		background: var(--color-background);
		border-radius: var(--ui-radius-small);
		border-left: 3px solid var(--color-interactivity);
	}
