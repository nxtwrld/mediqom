...
function classifyMuscle(name: string): MuscleGroup {
	...
	// Jaw / mastication
	if (/masseter|temporalis|pterygoid|digastric|mylohyoid|hyoglossus|stylohyoid/.test(n))
		return 'head_jaw';
	...
	// Shoulder
	if (/deltoi[td]|supraspinatus|infraspinatus|teres_(major|minor)|subscapularis|subclavius/.test(n))
		return 'shoulder';
	...
	// Torso posterior
	if (/latissimus|rhomboid|trapezius|erector|longissimus|iliocostalis|spinalis/.test(n))
		return 'torso_posterior';

	// Torso deep
	if (/internal_oblique|transverse_abdominis|intercostal|diaphragm|quadratus_lumborum/.test(n))
		return 'torso_deep';
	...
	if (/rectus_femoris|vastus|sartorius|pectineus/.test(n)) return 'leg_anterior';
	...
	// Foot
	if (/abductor_hallucis|abductor_digiti_minimi_foot|flexor_digitorum_brevis|extensor_digitorum_brevis/.test(n))
		return 'foot';
 ...
 */
export function createMuscleMaterial(muscleName: string): THREE.MeshPhysicalMaterial {
